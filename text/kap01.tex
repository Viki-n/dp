...
konečnou podmnožinu tohoto univerza $M$. Budeme chtít vybudovat datovou strukturu, která dokáže o
každém prvku $\mathcal U$ říct, zda náleží množině $M$. Po takových
...
prvek. Tomuto vrcholu nutně chybí přinejmenším levý syn (kdyby existoval, hodnota v
...
optimálním stromu je sám o sobě optimálním stromem nad příslušnými prvky. Proto
...
Nyní se pokusíme odhadnout minimální počet vrcholů $D(n)$ ve stromu hloubky
...
výšek stromů (tyto černé výšky nemusí být po dvou různé). Dále platí, že v
seznamu prvků větších než $x$ je hodnota každého vrcholu menší než hodnota všech
...
nejlepší možné konfiguraci. Tím však nesnížíme čas potřebný na přístupy o více
...
Samotná operace $\ope{Splay}(x)$ se skládá ze dvou fází: Nejprve standardním způsobe
...
\emph{zřetězením} posloupností $S$ a $T$ budeme rozumět posloupnost $S\|T$ o
$m+t$ prvcích takovou, že $\forall i\leq m: (S\|T)_i = S_i$ a $\forall i,
...
Aby bylo možné větě porozumět, musíme si uvědomit, že strom $T$ použijeme pouze k
...
splay stromem a v následující kapitole popsaným multisplay stromem) jednou z
...
Následně v levém podstromu $v$ najdeme pomocí informace o minimální hloubce v
podstromu daného vrcholu vrchol $\ell$, který je vrcholem s nejvyšší hodnotou v
tomto stromu s hloubkou menší než $v$. Pak zavoláme \ope{Splay$(\ell)$} na levý
...
$P$ má hloubku $\Theta(\log n)$. Protože o $P$ jsme dosud mluvili jako o
...
kořene některého pomocného stromu v $T$. Tyto kořeny najdeme tak, že budeme z
...
vyřešíme přepočítání informace o minimální hloubce v podstromu -- všimneme si, že v
...
stromu, máme vše i pro implementaci operace \ope{Delete} nad multisplay
