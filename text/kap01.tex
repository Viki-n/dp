...
Splay strom představili \citet{splay}. Jedná se o strom, který modifikuje svou strukturu nejen při operacích Insert a Delete, ale i při vyhledávání\footnote{Přestože splay strom vkládání i mazání podporuje, my se nyní zabýváme modelem, kde stromy umí pouze vyhledávat. Proto budeme předpokládat, že operaci Delete nikdy nevykonáme, a všechny operace Insert vykonáme před začátkem měření v rámci inicializace struktury.}. Splay strom podporuje operaci $Splay(x)$, a všechny ostatní stromové operace implementuje pomocí $\o(1)$ volání této operace.

Samotná operace $Splay(x)$ se sestává ze dvou fází: Nejprve standardním způsobe
...
rodič syni na téže či různé straně použijeme buď zig-zig rotaci, nebo zig-zag
...
Pomocí operace $Splay(x)$ naimplementujeme ostatní stromové operace. Při operaci Insert vložíme prvek do stromu standardním způsobem, a poté na něj aplikujeme $Splay$. Při operaci Delete opět prvek odstraníme standardním způsobem, a potom na nejhlubší prvek, kterého jsme se dotkli a který ve stromě zůstal, aplikujeme $Splay$. Operaci join tak, jak byla popsána v kapitole \nameref{sec:RB}, můžeme provést tak, že ve stromu, který bez újmy na obecnosti obsahuje menší prvky ze dvou spojovaných stromů, zavoláme $Splay(\infty)$. Tím dostaneme nejvyšší prvek do kořene. Kořeni proto bude chybět pravý syn. Na jeho místo tedy můžeme připojit druhý strom. Pokud máme v rámci spojování prvek mezi hodnotami obou stromů do stromu vložit, stačí ho prohlásit za nový kořen a jako syny mu připojit kořeny původních stromů. Split provedeme následovně:

\begin{enumerate}
\item $Split(x)$ v našem případě rozdělí strom na stromy s hodnotou klíče větší než $x$, menší než $x$, a případně nechá samostatně vrchol s hodnotou $x$, pokud ve stromě byl. Začneme tedy zavoláním $Splay(x)$.
...
Splay strom nám garantuje logaritmickou amortizovanou složitost operace $Splay$. V některých situacích může však být i lineárně hluboký. Pokud například přistoupíme ke všem prvkům v rostoucím pořadí, bude mít strom tvar cesty, kde každý vrchol (krom jediného listu) bude mít pouze levého syna. Pokud poté zavoláme $Splay$ na nejmenší prvek ve stromě, bude trvat lineárně dlouho, avšak hloubku stromu zmenší o polovinu.
...
Výše uvedená definice optimality bohužel není příliš užitečná, protože není znám algoritmus, který by efektivně spočítal chování optimálního binárního vyhledávacího stromu. Proto různí autoři přišli s různými odhady na $\opt(S)$. My zde předvedeme několik horních odhadů a jeden dolní odhad. Začneme třemi odhady, které představili \citet{splay}. Ti ve svém článku rovnou ukázali, že všechny tři meze (až na aditivní člen závisející pouze na $n$) splay strom splňuje. Proto musí platit i pro optimální strom.
...
struktura, která umožňuje udržovat nějaké množství \emph{prstů}, z nichž kadý
...
Mějme přístupovou posloupnost $S$ délky $m$ nad množinou $[n]$ jejíž $j$-tý prvek si označíme $i_j$. Potom pro platí $$\opt(S) = \o\left(m + \sum_{j=2}^m \log(1 + |i_{j}-i_{j-1}|) \right).$$
\end{veta}
...
Mějme přístupovou posloupnost $S$ nad množinou $[n]$. Dále mějme libovolný (statický) strom $T$ nad toutéž množinou klíčů. Každý vrchol $T$ má svůj preferovaný směr. Vždy, když projdeme vrcholem $v$, nastavíme preferovaný směr $v$ na syna $v$, do nějž jsme z vrcholu $v$ pokračovali. $\ib_T(S)$ označíme celkový počet změn preferovaných směrů\footnote{Protože nás zajímají pouze posloupnosti $S$ takové, že jejich délka $m$ je větší nebo rovna $n$, počáteční nastavení preferovaných směrů ovlivní $\ib_T(S)$ nejvýše o multiplikativní konstantu.} ve stromě $T$ při vykonání posloupnosti přístupů $S$. Pak pro libovolný strom $T$ platí $$\opt(S) = \Omega(\ib_T(S)).$$
\end{veta}

Interleave bound představili a dokázali v tomto znění \citet{tango}. Až na multiplikativní konstantu stejnou mez ale představil už \citet{interleave}. Tato mez nám dává mimo jiné rodinu konkrétních posloupností $S$ takových, že $\opt(S)=\Theta(m\log n)$. Pokud jako strom $T$ zvolíme perfektně vyvážený binární strom, můžeme přistupovat k prvkům v jeho listech v takovém pořadí, abychom vždy v každém procházeném vrcholu změnili preferovaný směr. Takovým posloupnostem se také říká \emph{bit reversal posloupnosti}. Na druhou stranu ZDROJ ukázal, že pro žádný pevný strom $T$ nedává Interleave bound těsný odhad pro ksždou přístupovou posloupnost $S$. Zda pro každou posloupnost $S$ existuje strom $T$ takový, aby $\ib_T(S)=\opt(S)$, není známo.  
...
bude zřejmé, jak sestrojit přístupovou posloupnost, kterou optimální strom
 vykoná v čase $\o(m)$, ale tango strom bude potřebovat
$\o(m\cdot\log\log(n))$.
...
$P$. Tyto pomocné stromy budou červenočerné stromy. Protože všechny cesty v $P$
jsou nejvýše logaritmicky dlouhé, bude každá operace (ať už search, nebo split
či join) s pomocnými stromy trvat $\o(\log\log(n))$ času, a pomocných stromů
